...
using System.Linq;
using static Fantastic.Utils.LambdaUtil;

namespace Fantastic.Xamarin.Example
{
    ...
    public class MainActivity : AppCompatActivity
    {
        private readonly List<Cocktail> cocktails =
            JsonConvert.DeserializeObject<List<Cocktail>>(data)
                       .OrderBy(a => a.Name)
                       .ToList();

        private FilterableAdapter<Cocktail> filterableAdapter;
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            var recycler = FindViewById<RecyclerView>(Resource.Id.recycler_view);

            var getImageSelector = fun(() =>
            {
                ...
                return fun(() => urls[random.Next(0, 12)]);
            });
            
            var picasso = Picasso.With(this);

            var selectImage = getImageSelector();

            var createView = fun((ViewGroup parent, int _) =>
            {
                var view = LayoutInflater.From(parent.Context).Inflate(Resource.Layout.item, parent, false);

                var cocktailName = view.FindViewById<TextView>(Resource.Id.cocktailName);
                var category = view.FindViewById<TextView>(Resource.Id.category);
                var preparation = view.FindViewById<TextView>(Resource.Id.preparation);
                var picture = view.FindViewById<ImageView>(Resource.Id.picture);
                var numberOfIngredients = view.FindViewById<TextView>(Resource.Id.numberOfIngredients);

                var updateView = act((Cocktail cocktail) =>
                {
                    cocktailName.Text = cocktail.Name;
                    category.Text = cocktail.Category;
                    preparation.Text = cocktail.Preparation;
                    numberOfIngredients.Text = $"#{cocktail.Ingredients.Count}";

                    const int pictureSize = 75;

                    picasso.Load(selectImage())
            ...
            filterableAdapter = 
                FilterableAdapter.Make(cocktails,
                                       createView,
                                       s => cocktails.Where(a => a.Name.StartsWith(s, StringComparison.OrdinalIgnoreCase))
                                                     .ToArray());
        ...
        public override bool OnCreateOptionsMenu(IMenu menu)
        {
            ...
            var searchView = (Android.Support.V7.Widget.SearchView)searchItem.ActionView;

            searchView.QueryTextChange +=
                 (_, q) =>
                 {
                     filterableAdapter.GetFilter().InvokeFilter(q.NewText);
